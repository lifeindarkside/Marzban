...
- Вы увидите логи которые вы можете остановить, нажав `Ctrl+C` или закрыв терминао.
- Файлы Marzban будут размещены по адресу `/opt/marzban`
- Файл конфигурации будет размещен по адресу `/opt/marzban/.env` (см.раздел [Конфигурация](#configuration))
...
Готово! Теперь Вы можете войти, используя свои данные.

Для того, что бы увидеть справочное сообщение скрипта Marzban, выполните команду

```bash
marzban --help
```

Если Вы хотите запустить проект, используя исходный код, обратитесь к разделу ниже
<details markdown="1">
...
В качестве альтернативы для создания изолированной среды можно использовать [Python Virtualenv](https://pypi.org/project/virtualenv/)
...
Если Вы хотите использовать `marzban-cli`, необходимо связать его с файлом в `$PATH`, сделать исполняемым и установить:
...
Создайте копию файла `.env.example`, посмотрите и отредактируйте его с помощью текстового редактора типа `nano`.
...
> Проверьте раздел  [Конфигурации](#configuration) для получения большей информации
...
| UVICORN_HOST                      | Привязка приложения к данному хосту (по умолчанию: `0.0.0.0`)                                                    |
| UVICORN_PORT                      | Привязка приложения к данному порту (по умолчанию: `8000`)                                                       |
...
| XRAY_ASSETS_PATH                  | Путь к папке с файлами ресурсов для Xray (файлы geoip.dat и geosite.dat) (по умолчанию: `/usr/local/share/xray`)                                                |
...
Всегда полезно регулярно создавать резервные копии файлов Marzban, чтобы предотвратить потерю данных в случае системных сбоев или случайного удаления. Ниже описаны шаги по созданию резервной копии Marzban:
...
Проект Marzban представляет [Marzban-node](https://github.com/gozargah/marzban-node), который помогает Вам в распределение инфраструктуры. С помощью Marzban-node можно распределить инфраструктуру по нескольким узлам, получив такие преимущества как высокая доступность, масштабируемость, гибкость. Marzban-node позволяет пользователям подключаться к различным серверам, предоставляя им гибкость в выборе вместо того, чтобы ограничиваться только одним сервером.
...
Мы ❤️‍🔥 участников проекта! Если вы хотите внести свой вклад, пожалуйста, ознакомьтесь с нашим [Contributing Guidelines](CONTRIBUTING.md) и не стесняйтесь отправить запрос на исправление или сообщить о проблеме. Мы также приглашаем вас присоединиться к нашей группе [Telegram](https://t.me/gozargah_marzban) для получения поддержки.

Проверьте [open issues](https://github.com/gozargah/marzban/issues), чтобы помочь развитию этого проекта.

<p align="center">
Спасибо всем участникам, которые помогли Marzban стать лучше:
</p>
<p align="center">
